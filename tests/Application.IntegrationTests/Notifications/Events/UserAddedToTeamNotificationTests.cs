...
public class UserAddedToTeamNotificationTests : BaseIntegrationTest
{
    public UserAddedToTeamNotificationTests(IntegrationTestWebAppFactory factory)
        : base(factory)
    {
    }

    [Fact]
    public async Task AddUserToTeam_WhenUserAdded_ShouldCreateNotificationForAddedUser()
    {
        // Arrange
        ...
        notification!.Type.Should().Be(NotificationType.AddedToTeam);
        notification.EntityId.Should().Be(team.Id);
        notification.Message.Should().Contain(team.Name);
    }
}
